...
public class RPSTrainer {
    ...
    private double[] getStrategy() {
        ...
        for (int a = 0; a < NUM_ACTIONS; a++) { //strategy[a] = max(regretSum[a], 0)
            strategy[a] = regretSum[a] > 0 ? regretSum[a] : 0;
            normalizingSum += strategy[a];
        }
        for (int a = 0; a < NUM_ACTIONS; a++) { //normalize strategy vector and add to strategySum
            if (normalizingSum > 0)
    ...
    public void train(int iterations) {
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            //set actionUtility vector based on RPS rules
    ...
    public double[] getAverageStrategy() { // returns normalized strategySum
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(trainer.regretSum));
    }
}